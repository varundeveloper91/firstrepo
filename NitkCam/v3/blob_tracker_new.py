...
while 1:    
    ...
    cv.Smooth(image, image_smoothed, cv.CV_GAUSSIAN, 7)
    # threshold the smoothed image
    image_threshed = thresholded_image(image_smoothed)
    
    # blobify
#    cv.Dilate(image_threshed, image_threshed, None, 10)
#    cv.Erode(image_threshed, image_threshed, None, 4)
...
#        print len(current_contour)
        ...
        largest_contour = current_contour.h_next()
        contourCount = 0
#        while True:
#            largest_contour = current_contour.h_next()
#            if (not largest_contour):
#                break
#            elif cv.ContourArea(largest_contour)>5:
...
#                cv.Circle(object_indicator, object_position, 5, (0,0,255), 4)
